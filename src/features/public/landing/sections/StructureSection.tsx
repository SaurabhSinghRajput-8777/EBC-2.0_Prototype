...
export function StructureSection() {
  return (
    <section id="bootcamp" className="py-20 sm:py-24 md:py-32 lg:py-8 md:py-10 bg-surface border-y border-primary/10 dark:border-white/5 relative overflow-hidden">
      ...
      <Container className="relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-6 md:mb-8 flex flex-col items-center">
          <MotionWrapper variant="slideUp" className="flex flex-col items-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-background border border-primary/10 dark:border-white/10 text-foreground text-xs sm:text-sm font-bold tracking-widest uppercase shadow-sm mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-6 lg:gap-8 relative">
          {siteContent.structure.steps.map((item, index) => (
            <MotionWrapper key={index} variant="slideUp" delay={0.1 * (index + 1)} className="h-full">
              <div className="group relative bg-background rounded-[24px] sm:rounded-[32px] p-2 sm:p-2.5 border border-primary/10 dark:border-white/10 shadow-md hover:shadow-2xl transition-all duration-500 overflow-hidden flex flex-col h-full">

                {/* Large Background Number */}
                <div className="absolute -top-6 -right-2 sm:-right-4 text-[120px] sm:text-[160px] font-black text-slate-50 dark:text-white/5 z-0 pointer-events-none transition-transform duration-700 group-hover:scale-110 group-hover:-translate-x-4">
                  0{index + 1}
                </div>
                ...
                <div className="px-4 sm:px-6 pb-6 sm:pb-8 relative z-10 flex-1 flex flex-col">
                  <h3 className="text-xl sm:text-2xl lg:text-3xl mb-3 sm:mb-4 font-bold text-foreground group-hover:text-primary transition-colors duration-300 tracking-tight">
                    {item.title}
                  </h3>
                  <p className="text-secondary-text text-sm sm:text-base leading-relaxed flex-1 font-medium">
                    {item.description}
                  </p>
                </div>
